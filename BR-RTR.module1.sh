...
# Функция для проверки конфигурации BR роутера
check_br_config() {
    local config_file="EcoRouterOS.conf"

    echo "Проверка конфигурации BR роутера в файле $config_file"
    ...
    for entry in "${BR_RTR_EXPECTED_LINES[@]}"; do
    ...
    for pool_pattern in "${BR_RTR_NAT_POOLS[@]}"; do
    ...
    if [[ $found_networks -ge $BR_RTR_OSPF_NETWORKS_COUNT_MIN && $found_networks -le $BR_RTR_OSPF_NETWORKS_COUNT_MAX ]]; then
        echo "✓ Найдено $found_networks OSPF network объявлений"
        add_result "OSPF networks ($found_networks шт.)" "OK"
    else
        echo "✗ ОШИБКА: Ожидалось ${BR_RTR_OSPF_NETWORKS_COUNT_MIN}-${BR_RTR_OSPF_NETWORKS_COUNT_MAX} OSPF network, найдено $found_networks"
        add_result "OSPF networks (${BR_RTR_OSPF_NETWORKS_COUNT_MIN}-${BR_RTR_OSPF_NETWORKS_COUNT_MAX} шт.)" "FAIL"
    fi

    echo "--- Проверка статических NAT правил ---"

    local br_nat_success=0
    for nat_rule in "${BR_RTR_NAT_STATIC[@]}"; do
    ...
    if [[ $br_nat_success -eq ${#BR_RTR_NAT_STATIC[@]} ]]; then
...
echo "Выполняется скрипт проверки BR-RTR"
apt-get install wget sshpass -y
grep -q "$BR_RTR_SSH_HOST" ~/.ssh/known_hosts 2>/dev/null || ssh-keyscan "$BR_RTR_SSH_HOST" >> ~/.ssh/known_hosts
if ! [ -f "$BR_RTR_EXPORT_SCRIPT" ]; then
    wget "$BR_RTR_EXPORT_URL"
fi
expect "$BR_RTR_EXPORT_SCRIPT"
tar -xvf "$BR_RTR_CONFIG_ARCHIVE" -C /root/
