...
br_server_script() {
    ...
    apt-get install wget sshpass -y
    if ! [ -f BR-RTR-Export-Config.exp ]; then
...
setup_ftp_server() {
    ...
    # Проверяем, запущен ли FTP сервер
...
EOF
    ...
    # Добавляем в fstab только если записи еще нет
...
# Настраиваем FTP сервер (с проверкой)
setup_ftp_server
