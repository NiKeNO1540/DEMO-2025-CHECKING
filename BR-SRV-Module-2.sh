...
run_check "Проверка доступности порта SSH на удаленном сервере" "timeout 10 nc -z -w 5 172.16.1.4 2026 && echo 'Порт 2026 доступен' || echo 'Порт 2026 недоступен'"
run_check "Проверка статуса Docker сервиса" "docker ps | grep testapp"
